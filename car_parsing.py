...
class OLX_CarParser(CarParser):
    ...
    def parse_html(self) -> parseStatus:
        ...
        details_list: list[str] = [
            t.text for t in car_data.find_all("li", class_="css-1r0si1e")
        ][1:]
...
class OTOMOTO_CarParser(CarParser):
    ...
    def parse_html(self) -> parseStatus:
        car_data = self.soup.find(
            "div", attrs={"data-testid": "content-details-section"}
        )
        if car_data is None or isinstance(car_data, bs4.NavigableString):
            self.parsed = parseStatus.UNSUCCESFULLY_PARSED
            return self.parsed
        car_data_list = self.soup.find_all(
            "div", attrs={"data-testid": "advert-details-item"}
        )
        details_list: list[tuple[str, str]] = [
            tuple(t.text for t in tag.find_all(re.compile("[p,a]")))
            for tag in car_data_list
        ]
        details_list: list[tuple[str, str]] = [
            (tuple_[0], "Unknown") if len(tuple_) != 2 else tuple_
            for tuple_ in details_list
        ]
        self.fill_details(details_list)
        self.parsed = parseStatus.SUCCESFULLY_PARSED
        return self.parsed
    ...
    def get_price(self) -> str:
        ...
        if (
            isinstance(price_tag, bs4.NavigableString)
            or price_tag is None
            or price_tag.parent is None
        ):
            return ""
